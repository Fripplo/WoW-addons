...
NauticusLichking = LibStub("AceAddon-3.0"):NewAddon("NauticusLichking", "AceEvent-3.0", "AceComm-3.0", "AceTimer-3.0")
...
-- object variables
NauticusLichking.DEFAULT_PREFIX = "NauticSync" -- do not change!
NauticusLichking.versionNum = 131 -- for comparison
NauticusLichking.lowestNameTime = "--"
NauticusLichking.tempText = ""
NauticusLichking.tempTextCount = 0
NauticusLichking.debug = false
NauticusLichking.iconRenderTimer = nil

NauticusLichking.broadcastChannels = {
...
local _options = {
	showminimapicon = {
		...
		order = 600,
		get = function()
			return NauticusLichking.db.profile.showMiniIcons
		end,
		set = function(info, val)
			NauticusLichking.db.profile.showMiniIcons = val
			if not val then
				...
				NauticusLichking:DrawMapIcons(false, true)
			end
		end,
	},
	showworldmapicon = {
		...
		order = 650,
		get = function()
			return NauticusLichking.db.profile.showWorldIcons
		end,
		set = function(info, val)
			NauticusLichking.db.profile.showWorldIcons = val
			if not val then
				for _, t in pairs(transports) do
				...
				NauticusLichking:DrawMapIcons(true, false)
			end
		end,
	},
	iconminisize = {
		...
		order = 400,
		get = function()
			return NauticusLichking.db.profile.miniIconSize
		end,
		set = function(info, val)
			NauticusLichking.db.profile.miniIconSize = val
	...
	iconworldsize = {
		...
		order = 500,
		get = function()
			return NauticusLichking.db.profile.worldIconSize
		end,
		set = function(info, val)
			NauticusLichking.db.profile.worldIconSize = val
			val = val * ICON_DEFAULT_SIZE
			for _, t in pairs(transports) do
	...
	iconframerate = {
		...
		order = 600,
		get = function()
			return NauticusLichking.db.profile.iconFramerate
		end,
		set = function(info, val)
			NauticusLichking.db.profile.iconFramerate = val
			if NauticusLichking.iconRenderTimer then
				NauticusLichking:CancelTimer(NauticusLichking.iconRenderTimer)
			end
			NauticusLichking.iconRenderTimer = NauticusLichking:ScheduleRepeatingTimer("DrawMapIcons", 1.0 / val, true, true)
		end,
		min = 1, max = 60, step = 1,
	},
	factiononly = {
		...
		order = 675,
		get = function()
			return NauticusLichking.db.profile.factionSpecific
		end,
		set = function(info, val)
			NauticusLichking.db.profile.factionSpecific = val
	...
	autoselect = {
		...
		order = 150,
		get = function()
			return NauticusLichking.db.char.autoSelect
		end,
		set = function(info, val)
			NauticusLichking.db.char.autoSelect = val
		end,
	},
	alarm = {
		type = 'range',
		...
		order = 300,
		get = function()
			return NauticusLichking.db.profile.alarmOffset
		end,
		set = function(info, val)
			NauticusLichking.db.profile.alarmOffset = val
		end,
		min = 0, max = 90, step = 5,
	},
	minibutton = {
		...
		order = 100,
		get = function()
			return not NauticusLichking.db.profile.minimap.hide
		end,
		set = function(info, val)
			NauticusLichking.db.profile.minimap.hide = not val
			if val then
				ldbicon:Show("NauticusLichking")
			else
				ldbicon:Hide("NauticusLichking")
			end
		end,
	},
	broadcastchannel = {
		...
		desc = format(L["Channel to broadcast your currently tracked transport (will broadcast to \"%s\" if the selected channel is unavailable)."], NauticusLichking.broadcastChannels["SAY"]),
		order = 350,
		values = NauticusLichking.broadcastChannels,
		get = function()
			return NauticusLichking.db.profile.broadcastChannel
		end,
		set = function(info, val)
			NauticusLichking.db.profile.broadcastChannel = val
			NauticusLichking:RefreshMenu()
		end,
	},
}
...
local optionsSlash = { type = 'group', name = "NauticusLichking", args = {
	[ L["icons"] ] = {
		type = 'group',
		name = L["Map Icons"],
...
NauticusLichking.optionsSlash = optionsSlash

function NauticusLichking:OnInitialize()
	self.db = LibStub("AceDB-3.0"):New("NauticusLichking5DB", defaults)
	LibStub("AceConfigRegistry-3.0"):RegisterOptionsTable("NauticusLichking", options)
	LibStub("AceConfig-3.0"):RegisterOptionsTable("NauticusLichkingSlashCommand", optionsSlash, { "nauticus", "naut" })
	options.args.NauticusLichkingSlashCommand = optionsSlash
	self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("NauticusLichking", nil, nil, "GUI")
	ldbicon:Register("NauticusLichking", self.dataobj, self.db.profile.minimap)

	local f = CreateFrame("Frame", "Naut_TransportSelectFrame", nil, "UIDropDownMenuTemplate")
	UIDropDownMenu_Initialize(f, function(frame, level) NauticusLichking:TransportSelectInitialise(frame, level); end, "MENU")
...
function NauticusLichking:GetBroadcastChannel()
	local channel = NauticusLichking.db.profile.broadcastChannel
	if (channel == "GUILD" and not IsInGuild()) or (channel == "PARTY" and not IsInGroup()) or (channel == "RAID" and not IsInRaid()) then
		channel = "SAY"
	end
	return channel, NauticusLichking.broadcastChannels[channel]
end

function NauticusLichking:OnEnable()
	self:RegisterEvent("ZONE_CHANGED_NEW_AREA")
	self:RegisterEvent("ZONE_CHANGED")
	self:RegisterEvent("PLAYER_ENTERING_WORLD")
	self:RegisterComm(self.DEFAULT_PREFIX)

	NauticusLichking.iconRenderTimer = self:ScheduleRepeatingTimer("DrawMapIcons", 1 / self.db.profile.iconFramerate, true, true)
...
function NauticusLichking:OnUpdate()
	...
	if prevx ~= 0 and prevy ~= 0 and prevdx == 0 and prevdy == 0 and (ddx > 0 or ddy > 0) and GetTime() - lastNonZeroDxDyTime > 10 then
		...
		-- 	NauticusLichking:CancelTimer(onUpdateTimer)
		-- end
		-- onUpdateTimer = NauticusLichking:ScheduleRepeatingTimer("CheckTriggers_OnUpdate", 1.0)
		-- NauticusLichking:CheckTriggers_OnUpdate()
		NauticusLichking:DebugMessage(format("MOVED: %.3f %d %d", GetTime(), time(), GetServerTime()))
	end
	--if prevx ~= 0 and prevy ~= 0 and (prevdx ~= 0 or prevdy ~= 0) and ddx == 0 and ddy == 0 then
	--	NauticusLichking:DebugMessage(format("STOPPED: %.3f", GetTime()))
	--end
...
function NauticusLichking:DrawMapIcons(renderWorldMapIcons, renderMinimapIcons)
...
function NauticusLichking:Clock_OnUpdate()
	if alarmDinged then
...
function NauticusLichking:CheckTriggers_OnUpdate()
...
function NauticusLichking:SetKnownTime(instanceID, transit, index, x, y, set)
...
function NauticusLichking:GetCycleByIndex(transit, index)
	return transitData[transit].offset[index]
end

-- initialise saved variables and data
function NauticusLichking:InitialiseConfig()
	...
	--local worldMapOverlay = CreateFrame("Frame", "NauticusLichkingWorldMapOverlay", WorldMapButton)
	...
	for id, data in pairs(transports) do
		...
		frame = CreateFrame("Button", "NauticusLichkingMiniIcon", Minimap)
		...
		frame:SetScript("OnEnter", function(self) NauticusLichking:MapIcon_OnEnter(self) end)
		frame:SetScript("OnLeave", function(self) NauticusLichking:MapIcon_OnLeave(self) end)
		frame:SetID(id)
		frame:Hide()

		frame = CreateFrame("Button", "NauticusLichkingWorldIcon", WorldMapFrame)
		...
		frame:SetScript("OnEnter", function(self) NauticusLichking:MapIcon_OnEnter(self) end)
		frame:SetScript("OnLeave", function(self) NauticusLichking:MapIcon_OnLeave(self) end)
		frame:SetScript("OnMouseDown", function(self) NauticusLichking:MapIcon_OnClick(self) end)
		frame:SetID(id)
		frame:Hide()
	end
...
function NauticusLichking:PLAYER_ENTERING_WORLD()
...
function NauticusLichking:UpdateZone(loopback)
...
function NauticusLichking:ZONE_CHANGED()
	self:UpdateZone(false)
end

function NauticusLichking:ZONE_CHANGED_NEW_AREA()
	self:UpdateZone(false)
end

function NauticusLichking:SetZone(zoneId)
...
function NauticusLichking:ToggleAlarm()
	alarmSet = not alarmSet
	if not alarmSet then alarmDinged = false end
	print(YELLOW.."NauticusLichking|r - "..WHITE..L["Alarm is now: "]..(alarmSet and RED..L["ON"] or GREEN..L["OFF"]).."|r")
	PlaySound(SOUNDKIT.AUCTION_WINDOW_OPEN)
end

function NauticusLichking:IsAlarmSet()
	return alarmSet or alarmDinged
end

function NauticusLichking:GetKnownCycle(transport)
...
function NauticusLichking:SetKnownCycle(transport, since, boots, swaps)
...
function NauticusLichking:HasKnownCycle(transport)
	local knownCycle = self.db.global.knownCycles[transport]
...
function NauticusLichking:GetFormattedTime(t)
	return formattedTimeCache[floor(t)]
end

function NauticusLichking:IsTransportListed(transportIndex)
...
function NauticusLichking:NextTransportInList()
...
function NauticusLichking:SetTransport(transport)
	if transport then
...
function NauticusLichking:DebugMessage(msg)
	if self.debug then
		local now = GetTime()
...
function NauticusLichking:sameSign(num1, num2)

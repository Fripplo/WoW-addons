...
function NauticusLichking:CancelRequest(distribution)
...
function NauticusLichking:DoRequest(wait, distribution)
...
do
	...
	function uncrunch(s)
		...
		for i = 1, strlen(s) do
			...
			if not c then
				NauticusLichking:DebugMessage("FECK: "..s.." ; i: "..i.." ; strbyte: "..strbyte(s, i))
				return
			end
			num = num + c * base
...
function NauticusLichking:BroadcastTransportData(distribution)
...
--[===[@debug@
function NauticusLichking:RequestAllTransports()
	local trans_str = ""
...
function NauticusLichking:RequestTransport(t, distribution)
	requestLists[distribution][t] = true
end

function NauticusLichking:SendMessage(msg, distribution)
...
function NauticusLichking:OnCommReceived(prefix, msg, distribution, sender)
...
function NauticusLichking:ReceiveMessage_version(clientversion, distribution, sender)
...
function NauticusLichking:ReceiveMessage_known(version, transports, hash, distribution, sender)
...
function NauticusLichking:IsBetter(transit, since, boots, swaps)
	local ourSince, ourBoots, ourSwaps = self:GetKnownCycle(transit)
...
function NauticusLichking:StringToKnown(transports)
...
function NauticusLichking:UpdateChannel(wait)
